...
const PartnerDashboard: React.FC = () => {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const partnerId = localStorage.getItem('auth_partner_id');
            if (partnerId) {
                const partnerData = await dbGetPartnerById(partnerId);
                setProfile(partnerData);

                if (partnerData?.status === 'active') {
                    const [statsData, campaignsData] = await Promise.all([
                        dbGetStats(partnerId),
                        dbGetPartnerCampaigns(partnerId)
                    ]);
                    setStats(statsData);
        ...
        <div className="min-h-screen bg-[#02020a] text-white flex">
            ...
            <aside className="fixed left-0 top-0 h-full w-64 bg-[#050510] border-r border-white/5 p-6 flex flex-col z-50">
                <div className="mb-10 px-2 flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="font-black italic uppercase tracking-tighter text-lg leading-none text-white">Partner</h1>
                        <p className="text-[8px] font-bold text-violet-400 uppercase tracking-widest">Growth Engine</p>
                    </div>
                </div>

                <nav className="flex-1 space-y-2">
                    <button onClick={() => navigate('/partner/dashboard')} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-violet-600 text-white shadow-lg shadow-violet-900/20">
                        <LayoutDashboard size={18} />
                        <span className="text-xs font-bold uppercase tracking-widest">Overview</span>
                    </button>
                    <button
                        onClick={() => !isLocked && navigate('/partner/campaigns')}
                        disabled={isLocked}
                    ...
                    <button
                        onClick={() => !isLocked && navigate('/partner/reports')}
                        disabled={isLocked}
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${isLocked ? 'text-white/10 cursor-not-allowed' : 'text-white/40 hover:bg-white/5 hover:text-white'}`}
                    >
                        <FileText size={18} />
                    ...
                    <button
                        onClick={() => !isLocked && navigate('/partner/billing')}
                        disabled={isLocked}
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${isLocked ? 'text-white/10 cursor-not-allowed' : 'text-white/40 hover:bg-white/5 hover:text-white'}`}
                    >
                        <CreditCard size={18} />
            ...
            <main className="flex-1 lg:ml-64 p-10">
                <header className="flex justify-between items-center mb-12">
                    <div>
                        <h2 className="text-4xl font-black italic uppercase tracking-tighter mb-2">Partner Hub</h2>
                    ...
                    {!isLocked && (
                        <button onClick={() => navigate('/partner/campaigns')} className="flex items-center gap-2 px-6 py-3 bg-violet-600 rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-violet-500 transition-all shadow-lg shadow-violet-900/40">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                            ...
                            <div className="bg-[#0A0A15] border border-white/5 rounded-[2.5rem] overflow-hidden shadow-xl">
                                <div className="p-8 border-b border-white/5 flex justify-between items-center">
                                    ...
                                    <button onClick={() => navigate('/partner/campaigns')} className="text-[10px] font-black text-violet-400 uppercase tracking-widest hover:text-white transition-all">View All</button>
                                </div>
                                <div className="p-8 space-y-4">
                                    {campaigns.length === 0 ? (
                                        ...
                                        campaigns.slice(0, 3).map(c => (
                                            <div key={c.id} onClick={() => navigate('/partner/campaigns')} className="flex items-center justify-between p-5 rounded-3xl bg-white/[0.02] border border-white/5 hover:bg-white/5 hover:border-violet-500/30 transition-all cursor-pointer group">
                                ...
                                {rejectedCampaigns.length > 0 && (
                                    <div className="bg-rose-500/10 border-2 border-rose-500/20 rounded-[2.5rem] p-10 flex flex-col items-center text-center space-y-6 animate-pulse">
                                        ...
                                        <div>
                                            ...
                                            <button
                                                onClick={() => navigate('/partner/campaigns')}
