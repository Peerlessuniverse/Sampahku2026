...
const PartnerCampaigns: React.FC = () => {
    ...
    const fetchData = async (id: string) => {
        setIsLoading(true);
        try {
            const advData = await dbGetPartnerById(id);
            setProfile(advData);

            if (advData?.status === 'active') {
                const data = await dbGetPartnerCampaigns(id);
                setCampaigns(data);
            }
        } catch (err) {
            console.error(err);
    ...
    const handleSubmitForReview = async (id: string) => {
        try {
            ...
            const partnerId = localStorage.getItem('auth_partner_id');
            if (partnerId) fetchData(partnerId);
        } catch (err) {
            console.error(err);
        }
    };
    ...
    if (isAccountLocked) {
        return (
            <div className="min-h-screen bg-[#02020a] text-white flex flex-col items-center justify-center p-6 text-center">
                ...
                <button onClick={() => navigate('/partner/dashboard')} className="mt-10 px-8 py-4 bg-white/5 border border-white/10 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-white/10 transition-all flex items-center gap-2">
